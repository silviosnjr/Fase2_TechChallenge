...
# geramos população aleatória
def generate_random_population(pontos_onibus_location: List[Tuple[float, float]], population_size: int) -> List[List[Tuple[float, float]]]:
    """
    ...
    """
    return [random.sample(pontos_onibus_location, len(pontos_onibus_location)) for _ in range(population_size)]
...
# metodo de cruzamento criação dos filhos
# através de dois individuaos criamos os filhos baseado em seus percursos
# serão preservados alguns desse percursos nos filhos
# misturando dois materiais genéticos para conseguir esse melhor desempenho 
def order_crossover(parent1: List[Tuple[float, float]], parent2: List[Tuple[float, float]]) -> List[Tuple[float, float]]:
    ...
    """
    # tamanho do individuo
    length = len(parent1)

    # sorteando dois números inteiros para copiar os percursos desse intervalo
    start_index = random.randint(0, length - 1)
    # o star é sempre menor
    end_index = random.randint(start_index + 1, length)

    # Inicializamos um filho com uma copia da substring do pai
    child = parent1[start_index:end_index]

    # Vamos pegar as posições complementares do pai 2 para o filho 1
    remaining_positions = [i for i in range(length) if i < start_index or i >= end_index]
    
    # Pegando os pontos de onibus que ainda não apareceram para colocar em filho
...
# A mutação retorna sempre um par e faz a inversão
# passamos para essa função uma mutação, e retorna com a mutação ou não, tendo uma probabilidade para sofre ou não mutação.
def mutate(solution:  List[Tuple[float, float]], mutation_probability: float) ->  List[Tuple[float, float]]:
    ...
    # Verifique se a mutação deve ocorrer
    if random.random() < mutation_probability:
        
        # Certifique-se de que haja pelo menos dois pontos_onibus para realizar uma troca
        # técnica de swap trocando os pontos de ônibus
        if len(solution) < 2:
            return solution
    
        # Selecione um índice aleatório (excluindo o último índice) para troca
        index = random.randint(0, len(solution) - 2)
        
        # Troque os pontos_onibus no índice selecionado e no próximo índice
...
if __name__ == '__main__':
    ...
    pontos_onibus_locations = [(random.randint(0, 100), random.randint(0, 100))
