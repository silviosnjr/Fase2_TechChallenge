...
def draw_pontos_onibus(screen: pygame.Surface, pontos_onibus_locations: List[Tuple[int, int]], rgb_color: Tuple[int, int, int], node_radius: int) -> None:
    """
    Draws circles representing pontos_onibus on the given Pygame screen.

    Parameters:
    - screen (pygame.Surface): The Pygame surface on which to draw the pontos_onibus.
    - pontos_onibus_locations (List[Tuple[int, int]]): List of (x, y) coordinates representing the locations of pontos_onibus.
    ...
    None
    """
    for city_location in pontos_onibus_locations:
...
def draw_text(screen: pygame.Surface, text: str, color: pygame.Color) -> None:
    ...
    pontos_onibus_locations = []  # Assuming you have this list defined somewhere
    text_position = (np.average(np.array(pontos_onibus_locations)[:, 0]), HEIGHT - 1.5 * font_size)
